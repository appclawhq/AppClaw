...
import { interactivePicker, type PickerItem } from '@appclaw/core/device/interactive-picker';
...
function mergePackageJson(abs: string, rel: string, dirName: string, results: WriteResult[]): void {
  const scripts = {
    test: 'appclaw-runner --env-file .env',
    'test:parallel': 'appclaw-runner --workers 3 --env-file .env',
  };
  const devDeps = {
    '@appclaw/runner': `^${MIN_APPCLAW_VERSION}`,
    '@appclaw/core': `^${MIN_APPCLAW_VERSION}`,
    tsx: '^4.21.0',
  };
  ...
  if (!pkg.devDependencies['@appclaw/runner'])
    pkg.devDependencies['@appclaw/runner'] = devDeps['@appclaw/runner'];
  if (!pkg.devDependencies['@appclaw/core'])
    pkg.devDependencies['@appclaw/core'] = devDeps['@appclaw/core'];
