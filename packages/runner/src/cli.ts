/**
 * The runner CLI. Reached two ways: the standalone `appclaw-runner` bin (what
 * scaffolded projects use), or the `appclaw test` umbrella subcommand.
 *
...
function printHelp(programName: string): void {
  ...
  console.log(`Usage: ${programName} [filter...] [options]
...
Examples:
  ${programName}
  ${programName} tests/login.spec.ts
  ${programName} --workers 3 --retries 1
  ${programName} --shard 1/4`);
}

/**
 * Run the test CLI. `programName` labels the help/usage text — the standalone
 * `appclaw-runner` bin passes its own name; the `appclaw test` umbrella keeps
 * the default so both entry points read naturally.
 */
export async function runCli(argv: string[], opts: { programName?: string } = {}): Promise<number> {
  const programName = opts.programName ?? 'appclaw test';
