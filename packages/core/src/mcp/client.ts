...
/**
 * Whether to print verbose MCP debug output ([appium-mcp] stderr stream and per-tool timing).
 * Re-evaluated on every call so SDK consumers can override `process.env.MCP_DEBUG` at runtime
 * (the AppClaw constructor flips this env var based on the `mcpDebug` option).
 */
function mcpDebug(): boolean {
