...
import { isInkActive, getHitlHandler } from '../ui/renderer.js';
...
export async function askUser(request: HITLRequest): Promise<HITLResponse> {
  ...
  // readline would fight Ink for raw-mode stdin. The CLI registers an Ink-based
  // handler via `setHitlHandler`, keeping Ink/React out of the core code path.
  if (isInkActive()) {
    const inkHandler = getHitlHandler();
    if (inkHandler) {
      return (await inkHandler(request)) as HITLResponse;
    }
  }
