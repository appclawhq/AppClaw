...
/**
 * Structural contract for the optional action recorder passed in by the CLI.
 * The concrete `ActionRecorder` (a CLI-owned, replay-recording class) lives in
 * the CLI package; core only needs the two methods it calls during the loop,
 * so it depends on this interface instead of the concrete class to keep the
 * engine free of CLI-only modules.
 */
export interface ActionRecorderLike {
  ...
  record(
    ...
    result: string,
    vision?: { description: string; coordinates: [number, number] }
  ): void;
}
...
async function executeMetaTool(
  ...
  async function scaleLLMCoords(tapX: number, tapY: number): Promise<{ x: number; y: number }> {
    try {
      ...
      const starkVision = (await import('@appclaw/vision')).default;
